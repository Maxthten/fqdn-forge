(() => {
  ...
  function exportControls(page, className = "analysis-actions") { return `<div class="${className}"><button class="secondary small" data-analysis-export="json" data-analysis-page="${page}">${t("exportJson")}</button><button class="secondary small" data-analysis-export="markdown" data-analysis-page="${page}">${t("exportMarkdown")}</button></div>`; }
  function diagnosticsItems(items) { return items.length ? `<details class="analysis-diagnostics"><summary>${t("errors")} (${items.length})</summary><ul>${items.map(item => `<li><code>${esc(item.code)}</code> · ${esc(item.object_id)} · ${esc(item.message)}</li>`).join("")}</ul></details>` : ""; }
  function diagnostics(document) { return diagnosticsItems(document?.diagnostics || []); }
  function mergedDiagnostics(...documents) { const unique = new Map(); for (const document of documents) for (const item of document?.diagnostics || []) unique.set(`${item.code}\u0000${item.object_id}\u0000${item.message}`, item); return diagnosticsItems([...unique.values()]); }
  ...
  function filters(page, fields) { return `<div class="toolbar analysis-filters">${fields.join("")}<button class="primary" data-analysis-action="load" data-analysis-page="${page}">${t("apply")}</button><button class="secondary" data-analysis-action="clear" data-analysis-page="${page}">${t("clear")}</button>${exportControls(page, "analysis-export-actions")}</div>`; }
  function table(headers, rows, className = "") { return rows.length ? `<div class="table-wrap ${className}"><table class="table"><thead><tr>${headers.map(header => `<th>${esc(header)}</th>`).join("")}</tr></thead><tbody>${rows.join("")}</tbody></table></div>` : `<div class="empty">${t("empty")}</div>`; }
  ...
  function formatTime(timestamp) { const date = new Date(timestamp); return Number.isNaN(date.valueOf()) ? esc(timestamp || "—") : esc(date.toLocaleString(context()?.state?.lang === "zh" ? "zh-CN" : "en-US", { dateStyle: "medium", timeStyle: "medium" })); }
  function shortId(value) { return esc(String(value || "—").slice(0, 8)); }
  function relatedSummary(cell) { const counts = [[cell.scenario_ids?.length || 0, "scenarioRecords"], [cell.campaign_ids?.length || 0, "campaignRecords"], [cell.exception_ids?.length || 0, "exceptionRecords"]]; return counts.filter(([count]) => count).map(([count, label]) => `${esc(count)} ${t(label)}`).join(" · ") || "—"; }
  ...
  function overview(document) {
    ...
    const notice = data.simulation_notice_code === "offline_artifacts_not_public_network_assets" ? t("offlineArtifactNotice") : data.simulation_notice || "";
    return `${pageTitle("analysis")}<section class="notice"><strong>${t("simulated")}</strong><p>${esc(notice)}</p></section><div class="grid analysis-grid"><div class="card"><div class="label">${t("reports")}</div><div class="metric">${esc(report.count || 0)}</div></div><div class="card"><div class="label">${t("coverage")}</div><div class="metric">${esc(counts.covered || 0)}</div><div class="muted">${esc(counts.missing || 0)} ${t("missing")} · ${esc(counts.excepted || 0)} ${t("excepted")}</div></div><div class="card"><div class="label">${t("replays")}</div><div class="metric">${esc(replay.count || 0)}</div><div class="muted">${esc(replay.mismatch_count || 0)} ${t("mismatch")}</div></div><div class="card"><div class="label">Campaign / Soak</div><div class="metric">${esc(data.campaigns?.count || 0)} / ${esc(data.soak?.count || 0)}</div></div></div><section class="card"><h2>${t("recentRuns")}</h2>${table([t("scenario"), t("status"), t("time"), t("requests"), t("retries")], (report.recent || []).map(item => `<tr><td class="recent-run"><strong>${esc(item.scenario_id)}</strong><code>${shortId(item.run_id)}</code></td><td>${status(item.status)}</td><td>${formatTime(item.finished_at)}</td><td>${esc(item.request_count)}</td><td>${esc(item.retry_count)}</td></tr>`))}</section><section class="two"><div class="card"><h2>Campaign</h2>${table([t("status"), t("seed"), t("failureCategories")], recentCampaigns.map(item => `<tr><td>${status(item.status)}</td><td>${esc(item.seed)}</td><td class="long-text">${esc(Object.entries(item.failure_categories || {}).map(([name, count]) => `${name}: ${count}`).join(", ") || "—")}</td></tr>`))}</div><div class="card"><h2>Soak</h2>${table([t("status"), t("operations"), t("concurrency")], recentSoaks.map(item => `<tr><td>${status(item.status)}</td><td>${esc(item.operations)}</td><td>${esc(item.concurrency)}</td></tr>`))}</div></section><section class="card"><h2>${t("failureCategories")}</h2><p class="long-text">${esc(failures)}</p></section>${diagnostics(document)}`;
  }
  ...
  function timelineTrend(document) {
    ...
    return `${pageTitle("timelineTrend")}${timelineControls}${truncated(timeline)}<section class="card"><div class="analysis-heading"><h2>${t("events")}</h2><button class="secondary small" data-analysis-action="timeline-view">${ui.timelineView === "timeline" ? t("table") : t("timeline")}</button></div><p class="muted">${esc(value(timeline, "data", {}).time_note || "")}</p>${ui.timelineView === "timeline" ? eventTimeline : eventTable}</section><section class="card"><div class="analysis-heading"><h2>${t("trends")}</h2><button class="secondary small" data-analysis-action="trend-view">${ui.trendView === "chart" ? t("table") : t("graph")}</button></div>${ui.trendView === "chart" ? trendChart : trendTable}<p class="muted">${esc(value(trends, "data", {}).unavailable?.coverage_gap_history || "")}</p></section>${mergedDiagnostics(timeline, trends)}`;
  }
  ...
  document.addEventListener("click", async event => {
    ...
    try {
      ...
      if (action.dataset.analysisAction === "coverage-detail") { ui.coverageDetail = action.dataset.analysisId; context()?.render?.(); requestAnimationFrame(() => { const detail = document.getElementById("coverage-detail"); detail?.scrollIntoView({ behavior: "smooth", block: "start" }); detail?.focus({ preventScroll: true }); }); return; }
  ...
  function onPageChange(page) { if (pages.has(page)) cache.delete(page); }
  window.FqdnForgeAnalysis = { pages, page: render, onPageChange };
})();
